#!/usr/bin/env python3
"""
Advanced RB Position Model Training Script
...
def load_rb_data(seasons=[2023, 2024]):
    """Load weekly RB data from NFL API."""
    print(f"Loading RB data for seasons: {seasons}")
    
    weekly_data = nfl.import_weekly_data(seasons)
    rb_data = weekly_data[weekly_data['position'] == 'RB'].copy()
    
    print(f"Loaded {len(rb_data)} RB weekly records")
    print(f"Available columns: {list(rb_data.columns)}")
    
    return rb_data

def prepare_features(rb_data):
    """Prepare features for modeling, excluding non-numeric columns."""
    print("Preparing features...")
    
    # Get numeric columns only
    numeric_cols = rb_data.select_dtypes(include=[np.number]).columns.tolist()
...
def create_derived_features(rb_data):
    """Create derived features for RB modeling."""
    print("Creating derived features...")
    
    # Efficiency metrics
    rb_data['yards_per_carry'] = (rb_data['rushing_yards'] / rb_data['carries']).fillna(0)
    rb_data['yards_per_reception'] = (rb_data['receiving_yards'] / rb_data['receptions']).fillna(0)
    rb_data['yards_per_touch'] = ((rb_data['rushing_yards'] + rb_data['receiving_yards']) / 
                                  (rb_data['carries'] + rb_data['receptions'])).fillna(0)
    
    # Total metrics
    rb_data['total_yards'] = rb_data['rushing_yards'] + rb_data['receiving_yards']
    rb_data['total_tds'] = rb_data['rushing_tds'] + rb_data['receiving_tds']
    rb_data['total_touches'] = rb_data['carries'] + rb_data['receptions']
    rb_data['touchdown_rate'] = (rb_data['total_tds'] / rb_data['total_touches']).fillna(0)
    
    # Season progression
    rb_data['early_season'] = (rb_data['week'] <= 4).astype(int)
    rb_data['mid_season'] = ((rb_data['week'] > 4) & (rb_data['week'] <= 12)).astype(int)
    rb_data['late_season'] = (rb_data['week'] > 12).astype(int)
    rb_data['week_progression'] = rb_data['week'] / 18
    
    # Cap extreme values to prevent model issues
    rb_data['yards_per_carry'] = rb_data['yards_per_carry'].clip(0, 15)
    rb_data['yards_per_reception'] = rb_data['yards_per_reception'].clip(0, 25)
    rb_data['yards_per_touch'] = rb_data['yards_per_touch'].clip(0, 20)
    rb_data['touchdown_rate'] = rb_data['touchdown_rate'].clip(0, 1)
    
    print("Derived features created")
    return rb_data

def encode_categorical_features(rb_data, feature_cols):
    ...
    rb_data['team_encoded'] = team_encoder.fit_transform(rb_data['recent_team'].fillna('UNK'))
    encoders['team'] = team_encoder
    
    # Encode opponent team
    opponent_encoder = LabelEncoder()
    rb_data['opponent_encoded'] = opponent_encoder.fit_transform(rb_data['opponent_team'].fillna('UNK'))
    encoders['opponent'] = opponent_encoder
    
    # Encode season type
    season_type_encoder = LabelEncoder()
    rb_data['season_type_encoded'] = season_type_encoder.fit_transform(rb_data['season_type'].fillna('REG'))
    ...
    return rb_data, feature_cols, encoders

def prepare_modeling_data(rb_encoded, feature_cols, target_col='fantasy_points'):
    """Prepare final modeling dataset."""
    print("Preparing modeling dataset...")
    
    # Select features and target
    X = rb_encoded[feature_cols].copy()
    y = rb_encoded[target_col].copy()
...
def main():
    ...
    print("=== ADVANCED RB MODEL TRAINING ===")
    ...
    output_dir = "PositionModel/RB_Model_Advanced"
    
    try:
        # Load and prepare data
        rb_data = load_rb_data(seasons)
        feature_cols = prepare_features(rb_data)
        rb_data = create_derived_features(rb_data)
        rb_encoded, feature_cols, encoders = encode_categorical_features(rb_data, feature_cols)
        
        # Prepare modeling dataset
        X, y = prepare_modeling_data(rb_encoded, feature_cols)
